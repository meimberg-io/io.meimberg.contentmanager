...
- Luxurize Manager importiert Bilder automatisch aus Microsoft OneDrive oder SharePoint, zeigt fünf neue Bilder mit Thumbnails an und ermöglicht den Import per Knopfdruck
...
- Für jedes Bild werden zwei Binaries gespeichert: ein hochauflösendes 6000x6000 Pixel Bild und ein 2000x2000 Pixel Thumbnail mit einem halbtransparenten "Luxurize"-Wasserzeichen
...
- In der Content-Enrichment-Phase erzeugt Luxurize Manager über OpenAI API automatisch Titel, Beschreibungen und Schlagwörter basierend auf konfigurierbaren Prompts
...
- Die Content-Übersicht zeigt an, welche Felder für jedes Bild vollständig sind, und erleichtert so die Qualitätskontrolle
...
- Nach Content-Enrichment lädt die App Bilder zu Gelato hoch, wo daraus Acrylbild-Produkte in den Größen 30x30, 40x40 und 50x50 cm erstellt werden
...
- Parallel veröffentlicht Luxurize Manager die Bilder auch als digitale Downloads auf Etsy, was eine zusätzliche Erlösquelle mit geringem Aufwand schafft
...
Die App integriert Social Media Publishing über Publer, um Marketing-Kanäle zentral zu bedienen und zu steuern.

- Luxurize Manager exportiert für jedes Bild automatisch Posts an Publer, die 5 Kanäle bedienen: Facebook, Instagram, Pinterest, X und Threads

    - Die Posts enthalten Bild, Caption, Titel, Tags und die Shop-URL, angepasst an die Besonderheiten jedes Kanals

    - Publer bietet einen Scheduling-Mechanismus, der auch automatisches Timing der Posts ermöglicht
    - Der Nutzer kann Post-Texte in Publer manuell anpassen, um kanal-spezifische Optimierungen vorzunehmen
    - Ein Status-Check "Publish to Publer" im Workflow signalisiert die Fertigstellung der Social Media Verteilung

- Die Content-Prompts für die Social Media Texte sind in der App konfigurierbar und KI-gestützt, um emotionale und lyrische Beschreibungen zu generieren

    - Diese Automatisierung sorgt für konsistente Markenbotschaften und spart erheblich Zeit im Social Media Management
    - Künftige API-Integration mit Publer könnte den Prozess komplett automatisieren und manuelle Eingriffe minimieren
...
- Storyblock wird als Backend verwendet, da es bereits Datenstrukturen und API-Zugriff für Bild- und Content-Speicherung bietet
    ...
    - Storyblock ermöglicht einfache Integration auf der bestehenden Website meimberg.io für Content-Display

- Die Nutzer-Authentifizierung erfolgt über Microsoft 365 OAuth, mit einer Whitelist für autorisierte Nutzer basierend auf E-Mail-Adressen
...
- Das Dashboard zeigt die Gesamtzahl der Bilder und deren Statusverteilung übersichtlich an, um schnelle Entscheidungen zu ermöglichen
    ...
    - Die UI unterstützt durch klare Visualisierung der fünf Status-Checks eine einfache Nachverfolgung und schnelle Workflow-Steuerung

- Status-Icons sind farblich differenziert (grün, gelb, rot/ grau) und in allen Ansichten sichtbar

    - Direkte Aktionen wie "Publish to Gelato", "Publish to Shop" oder "Publish to Publer" sind als Buttons integriert
...
- Erstellung eines detaillierten Anforderungskonzepts inklusive Maskenbeschreibungen, Use-Cases und Geschäftslogik für die App
- Recherche und Entscheidung über Backend-Technologie (Storyblock, Supabase oder Spring Boot) und Klärung, wo die Binaries gespeichert werden
- Implementierung einer Maske zum Importieren von Bildern aus Cloud-Speicher (OneDrive/SharePoint) mit Vorschau und Auswahlmöglichkeit
- Implementierung der Statusverwaltung für Bilder mit fünf Prüf-Häkchen und Automatisierung möglicher Uploads zu Gelato, Shopify und Publer
- Entwicklung der AI-Integration zur automatischen Generierung von Titel, Caption und Tags per OpenAI API, inklusive Anpassungs- und Neugenerierungsfunktion
- Implementierung von automatischer Thumbnail-Erzeugung mit Wasserzeichen und Speicherung beider Bildversionen im Backend
- Konzeption und Umsetzung der Social Media Integration über Publer mit automatischem Scheduling und Kanalspezifischer Textanpassung
- Sicherstellung einer OAuth-basierten Authentifizierung mit Microsoft 365 und Absicherung der API-Zugriffe
- Aufbau eines übersichtlichen Dashboards zur Statusanzeige aller Bilder mit Filter- und Suchfunktion und Detailansichten für die Bildbearbeitung
